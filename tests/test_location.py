...
class LocationTests(BaseTestCase):
    ...
    def _start_active_trip(self, username='testdriver', password='driverpass', destino='Rota de Teste', horario='Agora'):
        """Helper to log in a driver and start a trip with new fields."""
        self.login(username, password)
        response = self.client.post('/start_trip', data={'destino': destino, 'horario_viagem': horario}, follow_redirects=True)
        ...
        with self.app.app_context(): 
            ...
            self.assertIsNotNone(active_trip, "Helper falhou ao criar uma viagem ativa.") # Translated
    ...
    def test_api_active_buses_locations_with_data(self):
        ...
        test_destino = "Destino API"
        test_horario = "09:00"
        expected_schedule_api = f"Destino: {test_destino} - Horário: {test_horario}"
        trip_id = self._start_active_trip(username='testdriver', password='driverpass', destino=test_destino, horario=test_horario)
        ...
        self.assertEqual(bus_data['driver_username'], 'testdriver') # Username in test DB
        self.assertEqual(bus_data['schedule'], expected_schedule_api)
    ...
    def test_api_active_buses_locations_trip_no_location(self):
        ...
        self._start_active_trip(username='testdriver', password='driverpass', destino='Rota Sem Localizacao', horario='10:00')
